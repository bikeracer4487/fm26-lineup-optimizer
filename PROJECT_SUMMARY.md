...
4. **players-current.csv**
   - Generated by data_manager.py from FM26 Players.xlsx
   - Contains complete player data with calculated ratings
   - Primary data file for all scripts
...
- DM positions: Uses `DM(L)` and `DM(R)` separately for better player differentiation
...
### Alternative DM Handling
If you have a DM(C) column and want to use that instead:
```python
('DM1', 'DM(C)'),  # Use DM(C) rating for both DM positions
('DM2', 'DM(C)'),
```
...
**Note:** The advanced scripts (`fm_match_ready_selector.py`) DO handle:
- Injuries/suspensions (filtered out automatically)
- Fatigue and condition (factored into effective ratings)
- Match sharpness (affects selection)
- Match importance-based selection (High priority = pure effectiveness, Low/Medium = rotation/development)

## Future Enhancement Ideas

- [ ] Consider player age and potential for long-term planning
- [x] ~~Weight by fitness/condition for match-day selection~~ *(Done in fm_match_ready_selector.py)*
- [ ] Factor in player morale
- [x] ~~Support multiple tactics (A-team vs rotation team)~~ *(Done via match importance levels)*
