...
import { RefreshCw, Users, UsersRound, TrendingUp } from 'lucide-react';
import { motion } from 'framer-motion';

interface SecondXITabProps {
  state: AppState;
}
...
export function SecondXITab({ state }: SecondXITabProps) {
  ...
  const secondXI = data?.secondXI || {};
  const teamRating = data?.teamRatings?.secondXIAverage || 0;
  const firstXIRating = data?.teamRatings?.firstXIAverage || 0;
  const ratingDrop = firstXIRating > 0 ? firstXIRating - teamRating : 0;
  ...
  Object.entries(secondXI).forEach(([position, player]) => {
    if (!player) return;
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            <UsersRound className="text-fm-teal" /> Second XI
          </h2>
          <p className="text-fm-light/50 text-sm mt-1">
            Rotation squad - best XI from remaining players after First XI selection
          </p>
        </div>
      ...
      {/* Team Rating Summary */}
      {teamRating > 0 && (
        <div className="bg-fm-surface/50 border border-fm-purple/30 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-fm-purple/20 rounded-full flex items-center justify-center">
            ...
            <div className="text-right space-y-1">
              <div>
                ...
                <div className="text-2xl font-bold text-fm-teal">{Object.keys(secondXI).length}/11</div>
              </div>
              {ratingDrop > 0 && (
                <div className="text-sm text-orange-400">
                  -{ratingDrop.toFixed(1)} from First XI
                </div>
              )}
            </div>
          </div>
      ...
      {loading && Object.keys(secondXI).length === 0 && (
      ...
      <div className="space-y-6">
        ...
        {!loading && Object.keys(secondXI).length === 0 && !error && (
          <div className="text-center text-fm-light/50 py-12">
            ...
            <div>Click Refresh to load the Second XI</div>
          </div>
        )}
      </div>
    </div>
...
function PlayerRow({ position, player }: { position: string, player: RotationPlayer }) {
  ...
  return (
    <motion.div
      ...
      className="bg-fm-surface p-3 md:p-4 rounded-lg border border-white/5 hover:border-fm-purple/30 transition-all"
    >
      {/* Mobile Layout */}
      <div className="md:hidden space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Badge className={`${hasDualPosition ? 'bg-pink-500/20 text-pink-400 border-pink-500/30' : 'bg-purple-500/20 text-purple-400 border-purple-500/30'} font-mono text-xs`}>
              {position}
            </Badge>
            <div>
          ...
          <div className="text-right">
            <div className="text-xl font-bold text-purple-400">{player.rating.toFixed(1)}</div>
          </div>
        </div>
      ...
      <div className="hidden md:grid md:grid-cols-[100px_1fr_80px_100px_60px_80px_80px_80px] gap-2 items-center">
        <Badge className={`${hasDualPosition ? 'bg-pink-500/20 text-pink-400 border-pink-500/30' : 'bg-purple-500/20 text-purple-400 border-purple-500/30'} font-mono justify-center text-xs`}>
          {position}
        </Badge>
